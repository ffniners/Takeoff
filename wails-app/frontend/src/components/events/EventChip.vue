<template>
  ...
  >
    <div class="flex items-center justify-between text-xs uppercase tracking-wide">
      <span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 font-semibold" :class="priorityClasses[event.priority]">
        <span class="h-2 w-2 rounded-full" :class="priorityDot[event.priority]"></span>
        {{ event.priority }}
      </span>
      <span class="rounded-full px-2 py-0.5" :class="statusClasses[event.status] ?? statusClasses.default">
        {{ statusLabel }}
      </span>
    </div>
