<template>
  <div class="flex flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-white/10 bg-white/5 px-8 py-16 text-center text-slate-400">
    <slot name="icon">
    ...
    <div class="space-y-2">
      <h3 class="text-lg font-semibold text-white">
        <slot name="title">Nothing to show yet</slot>
      </h3>
      <p class="max-w-sm text-sm text-slate-400">
        <slot name="description">Adjust your filters or add a new event to get started.</slot>
      </p>
    </div>
    <slot name="action" />
  </div>
